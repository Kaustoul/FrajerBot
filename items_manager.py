...
from constants import SONGS_DATA_JSON_PATH, GEYSER_MAPPINGS_PATH, GEYSER_MAPPINGS_OUT_PATH, OUT_PATH
...
def geyser_mappings_add_record_titles(mappings_data):
    ...
    for custom_model_data, value in data.entries():
...
def move_geyser_pack():
    shutil.move(os.path.join("target", "geyser_resources.mcpack"), os.path.join(OUT_PATH, "geyser_resources.mcpack"))