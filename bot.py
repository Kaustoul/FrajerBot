...
from constants import SONGS_CONTENT_PATH, SONGS_COVERS_PATH, GUILD_ID_OBJECT, JAVA_PACK_OUT_PATH, SERVER_RCON_PORT, SERVER_RCON_PWD, SERVER_DATAPACK_NAME, BEDROCK_PACK_OUT_PATH, BEDROCK_PACK_OUT_NAME, GEYSER_MAPPINGS_OUT_PATH, GEYSER_MAPPINGS_NAME
from songs_manager import load_song_data, save_song_data, gen_songs_pack, merge_songs, prepare_out_folder, gen_packs, run_converter, create_custom_items_yaml, update_json_file
...
async def updaterp(ctx: discord.Interaction):
    ...
    song_data = gen_songs_pack()
    await res.desc("Merging records pack into the resource pack")
    merge_songs()
    #update_json_file(song_data, os.path.join("resources", "javarp", "sprites.json"))
    ...
    mappings_dict = update_geyser_mappings()
    move_geyser_pack()
    ...
    yaml_dict = {"MUSIC_DISC_11": yaml_dict}
    ...
    print("Sending Bedrock pack")
    ftp.upload(BEDROCK_PACK_OUT_PATH, os.path.join(ftp_path, "RadkuvPlugin", "data", BEDROCK_PACK_OUT_NAME))
    print("Sending geyser mappings")
    ftp.upload_json(os.path.join(ftp_path, "Geyser-Spigot", "custom_mappings", GEYSER_MAPPINGS_NAME), mappings_dict)
    print("Sending custom-items.yml")
    ftp.upload(os.path.join("resources", "custom-items.yml"), os.path.join(ftp_path, "RadkuvPlugin", "custom", "custom-items.yml"))
    print("Sending custom-discs.yml", yaml_dict)
    ftp.upload_yaml(os.path.join(ftp_path, "RadkuvPlugin", "custom", "custom-discs.yml"), yaml_dict)
    ftp.close()
